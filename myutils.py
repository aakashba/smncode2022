...
class batch_gen(tensorflow.keras.utils.Sequence):
    ...
    def divideseqs(self, batchfidlocs):
        ...
        for fidloc in batchfidlocs:
            ...
            if with_codevec:
                wcodevec = self.extradata['codevec'][str(fid)][0]
                wcodevec = wcodevec[:self.config['tdatlen'],:]
                tmp = np.zeros((self.config['tdatlen'],768),dtype=np.float64)
                tmp[:wcodevec.shape[0],] = wcodevec
                wcodevec = np.float64(tmp)
